<?php
    ...
    if(isset($_POST['sbm']) && !empty($_POST['search'])){
        $search = $_POST['search'];
        $sql = "SELECT * FROM quanlysanpham INNER JOIN thuonghieu ON quanlysanpham.th_id = thuonghieu.th_id WHERE tensp LIKE '%$search%'";
        $query = mysqli_query($conn, $sql);
        $total_prd = mysqli_num_rows($query);
    }else{
        $sql = "SELECT * FROM quanlysanpham inner join thuonghieu on quanlysanpham.th_id = thuonghieu.th_id";
        $query = mysqli_query($conn, $sql);
    }
...
<body>
            ...
            <div class="body-left">
                <div class="body-left-list">
                    <ul class="body-left-list-ul">
                        ...
                        <li class="body-left-list-ul-li"><a class="body-left-list-iteam" href="./Qlsanpham1.php"><i class="fas fa-table body-left-list-iteam-icon"></i><b>Sản Phẩm</b></a></li>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <button style="float: right; margin: 10px 0;" class="btn btn-success" onclick="window.open('Addsp.php', '_self')">Thêm sản phẩm</button>
            <?php
                if(isset($total_prd)){
                    if($total_prd !==0){
                    ...
                    <?php
                        ...
                        while ($row = mysqli_fetch_assoc($query)) {?>
                            <tr>
                                ...
                                <td>
                                    <img src="../public/IMG/Sanpham/Nike/<?php echo $row['hinhanh']; ?>" width="100">
                                
                                </td>

                                <td><?php echo $row['gia']; ?></td>
                                <td style="display: flex;">
                                    <a style="margin-right: 5px;" class="btn btn-warning" href="sanphamindex.php?page_layout=sua&id=<?php echo $row['sp_id']; ?>">Sửa</a>

                                    <a onclick="return Del('<?php  echo $row['tensp'];?>')" class="btn btn-danger" href="sanphamindex.php?page_layout=xoa&id=<?php echo $row['sp_id']; ?>">Xóa</a>
                                </td>
                            </tr>
                        <?php } ?>
                </tbody>
        ...
        <button class="btn btn-success" onclick="window.open('Addsp.php', '_self')">Thêm sản phẩm</button>
